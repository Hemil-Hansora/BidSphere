...
const PaymentProofs = () => {
  ...
  useEffect(() => {
    // Only open drawer when we have valid data
            ...
            {paymentProofs && paymentProofs.length > 0 ? (
              paymentProofs.map((element, index) => (
                <tr key={index}>
                  {console.log("---------------- ")}
                  {console.log(element)}
                  <td className="py-2 px-4 text-center">{element.user.username}</td>
...
export const Drawer = ({ setOpenDrawer, openDrawer }) => {
  ...
  // Safe initialization of local state
  ...
  const hasValidData = singlePaymentProof && Object.keys(singlePaymentProof).length > 0;

  return (
    <>
      <section 
        className={`fixed ${openDrawer ? "bottom-0" : "-bottom-full"} left-0 w-full transition-all duration-300 h-full bg-[#00000087] flex items-end`}
      >
          ...
          <div className="w-full px-5 py-8 sm:max-w-[640px] sm:m-auto">
            ...
            <form className="flex flex-col gap-5 my-5">
              <div className="flex flex-col gap-3">
                ...
                <input
                  type="text"
                  value={hasValidData ? singlePaymentProof.user.username  || "" : ""}
                  disabled
              ...
              <div className="flex flex-col gap-3">
                ...
                <textarea
                  rows={5}
                  value={hasValidData ? singlePaymentProof.comment || "" : ""}
                  disabled
                  className="text-xl px-1 py-2 bg-transparent border-[1px] border-stone-600 rounded-md focus:outline-none text-stone-600"
                />
              </div>
              {hasValidData && singlePaymentProof.proof && (
                <div>
                  <Link
                    to={singlePaymentProof.proof}
